/* ============================================================
   ...
   Demo login:  admin@demo.com / demo123   |   manager: manager@demo.com / manager123
   ============================================================ */
...
function createDemoClient() {
  ...
  const auth = {
    async getSession() {
      ...
      return { data: { session: v ? { user: { app_metadata: v === 'manager' ? { role: 'manager' } : {} } } : null } };
    },
    async signInWithPassword({ email, password }) {
      const e = String(email).trim().toLowerCase();
      if (e === DEMO_EMAIL && password === DEMO_PASSWORD) { sessionStorage.setItem(AUTH, '1'); return { data: {}, error: null }; }
      if (e === 'manager@demo.com' && password === 'manager123') { sessionStorage.setItem(AUTH, 'manager'); return { data: {}, error: null }; }
